    ...
    <article className="product-card bg-surface-container-lowest border border-outline-variant/30 rounded-lg flex flex-col h-full overflow-hidden shadow-sm md:shadow-none md:p-md">
      ...
      <div className="p-4 md:p-0 flex flex-col flex-grow gap-2 md:gap-0">
        ...
        <p className="text-on-surface-variant font-body-md text-body-md mb-0 md:mb-md flex-grow line-clamp-2 md:line-clamp-none">
          <span className="md:hidden">{`${product.nameType} ${product?.quantityPerBox && product.quantityPerBox > 1 ? `Thùng ${product.quantityPerBox} ${product?.typeBottle?.nameTypeBottle}, ${product.capacity}ml/chai` : ""}`}</span>
          <span className="hidden md:inline">{`${product.nameType} ${product?.quantityPerBox && product.quantityPerBox > 1 ? `Thùng ${product.quantityPerBox} ${product?.typeBottle?.nameTypeBottle}, ${product.capacity}ml/chai` : ""}`}</span>
        </p>

        <div className="flex flex-wrap gap-2 mt-2 md:hidden">
          <span className="bg-surface-container-high text-primary font-label-sm text-label-sm px-2 py-1 rounded">
            {product?.quantityPerBox && product.quantityPerBox > 1
              ? `${product.quantityPerBox}x${product.capacity}ml`
              : `${product.capacity}ml`}
          </span>
        </div>
